...
func (r *ApplicationReconciler) reconcileService(ctx context.Context, application *appv1.Application) (ctrl.Result, error) {
	...
	if !errors.IsNotFound(err) {
		log.Error(err, "Failed to get the Service,will request after a short time.", "ServiceNamespace", appNamespace, "ServiceName", appName)
		return ctrl.Result{RequeueAfter: GenericRequeueDuration}, err
	}
	svc.SetName(appName)
	...
	if err := ctrl.SetControllerReference(application, svc, r.Scheme); err != nil {
		log.Error(err, "Failed to set the owner reference for the Service.", "ServiceNamespace", appNamespace, "ServiceName", appName)
		return ctrl.Result{RequeueAfter: GenericRequeueDuration}, err
	}
